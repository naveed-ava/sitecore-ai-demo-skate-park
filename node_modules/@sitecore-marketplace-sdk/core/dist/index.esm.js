...
})(AllowedTouchpoints || (AllowedTouchpoints = {}));
/**
...
})(AllowedExtensionPoints || (AllowedExtensionPoints = {}));
...
class PostMessageBridge {
    ...
    setTarget(target) {
        if (!target) {
            throw this.formatError(new CoreError(ErrorCode.INVALID_REQUEST, 'Target window cannot be null or undefined'));
        }
    ...
    async connect() {
        ...
        if (!this.handshakeManager) {
            throw this.formatError(new CoreError(ErrorCode.INITIALIZATION_ERROR, 'Handshake manager not initialized'));
        }
        // For the client, we need to send the handshake init message
        if (this.sdkType === 'client') {
            if (this.listenerMode || !this.config.target) {
                throw this.formatError(new CoreError(ErrorCode.INVALID_STATE, 'Cannot connect in listener mode without a target. Set target first.'));
            }
            try {
    ...
    postMessage(message) {
        if (this.listenerMode || !this.config.target) {
            throw this.formatError(new CoreError(ErrorCode.INVALID_STATE, 'Cannot send messages in listener mode. Set target first.'));
        }
    ...
    ensureConnected() {
        ...
        if (this.listenerMode || !this.config.target) {
            throw this.formatError(new CoreError(ErrorCode.INVALID_STATE, 'Cannot perform operations in listener mode. Set target first.'));
        }
        if (!this.connected && this.sdkType === 'client') {
            throw this.formatError(new CoreError(ErrorCode.NOT_CONNECTED, 'Bridge is not connected. Call connect() first.'));
        }
    }
    /**
...
export { AllowedExtensionPoints, AllowedTouchpoints, CoreError, CoreSDK, ErrorCode, GenericResponseData };
